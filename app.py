...
REGION_COLOURS = {
    "Sub-Saharan Africa":         TERRACOTTA,
    "North Africa & Middle East": ORANGE,
    "South Asia":                 SAND,
    "East Asia & Pacific":        TEAL,
    "Europe & Central Asia":      TEAL_DARK,
    "Latin America & Caribbean":  TEAL_LIGHT,
    "North America":              "#8B6914",
    "Other":                      "#AAAAAA",
}
...
<style>
    ...
    .stApp li, .stMarkdown p, .stMarkdown li,
    .stCaption p, .stAlert p,
    ...
    [data-testid="stHeadingWithActionElements"] h3 { color: #111111 !important; }
    ...
    [data-testid="stSidebar"] div { color: #F5EFE6 !important; }
...
AXIS = dict(
    showgrid=True, gridcolor=GRID, linecolor=GRID,
    tickcolor=BLACK,
    tickfont=dict(color=BLACK),
    title_font=dict(color=BLACK),
)
# Reusable legend style — includes title font so "Region"/"Country" label is black
def legend_style(orientation="v", x=1.01):
    return dict(
...
fig_map = px.choropleth(
    ...
    color_continuous_scale=[
        [0.00, "#1A0A04"], [0.20, TERRACOTTA], [0.50, ORANGE],
        [0.75, SAND],      [1.00, TEAL],
    ],
    range_color=[0, 100],
...
fig_race.update_layout(
    **LAYOUT,
    ...
    xaxis=dict(
        **AXIS,
        title=dict(text="Percentage-Point Gain", font=dict(color=BLACK)),
        ticksuffix="pp",
    ),
    legend=dict(
...
fig_scatter.update_layout(
    **LAYOUT,
    ...
    legend=dict(
        ...
        font=dict(color=BLACK),
        title=dict(text="Region", font=dict(color=BLACK)),
    ),
)
...
region_trend = dff.groupby(["year","region"])["elec_access"].mean().reset_index()
    ...
    fig_spot.update_layout(
        **LAYOUT,
        ...
        legend=dict(
            orientation="v", x=1.01,
            font=dict(color=BLACK),
            title=dict(text="Country", font=dict(color=BLACK)),
        ),
    )
